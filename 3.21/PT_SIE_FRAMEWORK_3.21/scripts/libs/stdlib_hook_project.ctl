...
void hook_openFaceplate(string dp)
{
  ...
  if (globalExists("FACEPLATELIB_LOADED") && (g_openFaceplateTriggerEvent == 1 || g_openFaceplateTriggerEvent == 3)) //for PT SIE template
  {
//   execScript("#uses \"classes/GUI/faceplate\"\nmain(){}");
    ...
    // open faceplate in Siemens PT framework dock window
    execScript(
      "int main(string sParams) {"
      "  GUIFaceplate::TriggerOpenFaceplate(sParams);"
      "}",
      makeDynString(),   // no $ parameters
      dp + "§" + dp + "§" + sPanel + "§" + dpTypeName(dp) + sParams + "§$STDLIB:1"
    );
  }
  ...
  else
  {
    ...
    int iX, iY = (iYPosRel + 10) / zoomFactor;

    if (fromScreenEdge < margin)    // on the right margin
    {
      iX = (iXPosRel + fromScreenEdge - margin) / zoomFactor;
    }

//        ChildPanelOn(sPanel, hook_faceplateWindowTitle(dp), dsParameter, iX, iY);

    dyn_anytype da = makeDynAnytype(myModuleName(),
                                    getPath(PANELS_REL_PATH, sPanel),
                                    myPanelName(),
                                    (string)hook_faceplateWindowTitle(dp),
                                    iX,
                                    iY,
                                    1.0,
                                    true,
